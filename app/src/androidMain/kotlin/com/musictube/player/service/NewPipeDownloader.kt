...
class NewPipeDownloader private constructor(private val client: OkHttpClient) : Downloader() {
    ...
    override fun execute(request: NewPipeRequest): Response {
        ...
        headers.forEach { (name, values) -> values.forEach { value -> requestBuilder.addHeader(name, value) } }
        ...
        response.headers.forEach { (name, value) -> responseHeaders.getOrPut(name) { mutableListOf() }.add(value) }

        return Response(response.code, response.message, responseHeaders, response.body?.string(), response.request.url.toString())
    }
}
